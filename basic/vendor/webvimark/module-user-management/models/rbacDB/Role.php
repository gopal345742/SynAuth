...
class Role extends AbstractItem {
    ...
    public static function getGroupedList($showAll = false) {
        $return_roles = [];
        $role_desc_array = ArrayHelper::map(Role::getAvailableRoles($showAll), 'name', 'description');

        $groups = [
            \app\helpers\Constants::$hierarchy['type']['BU'] => ['HLOwner', 'AstGrpOwner', 'BusinessUser'],
            \app\helpers\Constants::$hierarchy['type']['TC'] => ['HLManager', 'AstGrpManager', 'AstMgr', 'AssociateMgr', 'IssueMgr'],
            'IT Security' => ['Infhead', 'Infmgr', 'Infcon', 'RiskMgr'],
            'Admin' => ['Admin']
        ];

        foreach ($groups as $groupname => $grouproles) {
            foreach ($role_desc_array as $rolecode => $roledesc) {
                if (in_array($rolecode, $grouproles)) {
                    $return_roles[] = ['rolecode' => $rolecode, 'roledesc' => $roledesc, 'group' => $groupname];
                }
            }
        }
        return $return_roles;
    ...
    //for RequestEcmWorkflowMapping
    public static function getNewGroupedList($showAll = false) {
        $return_roles = [];
        $role_desc_array = ArrayHelper::map(Role::getAvailableRoles($showAll), 'name', 'description');

        $new_roles = [
            'TC User' => 'Hierarchy User',
            'BU User' => 'Business User'
        ];
        $role_desc_array = array_merge($role_desc_array, $new_roles);

        $groups = [
            \app\helpers\Constants::$hierarchy['type']['BU'] => ['HLOwner', 'AstGrpOwner', 'BusinessUser', 'BU User'],
            \app\helpers\Constants::$hierarchy['type']['TC'] => ['HLManager', 'AstGrpManager', 'AstMgr', 'AssociateMgr', 'IssueMgr', 'TC User'],
            'IT Security' => ['Infhead', 'Infmgr', 'Infcon', 'RiskMgr'],
            'Admin' => ['Admin']
        ];
    ...
    public static function getRoleDescription($name) {
        if ($name == 'BU User' || $name == 'TC User') {
            
            return ($name == 'BU User') ? 'Business User' : 'Technology User';
        } else {
            $roles = ArrayHelper::map(static::getAvailableRoles(TRUE), 'name', 'description');
            if (isset($roles[$name])) {
                return $roles[$name];
            }
            return NULL;
