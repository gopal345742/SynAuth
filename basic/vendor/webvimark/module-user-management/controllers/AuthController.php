...
class AuthController extends BaseController {
    ...
    public function actionLogin() {
        ...
        $license_status = \app\components\utility\LicenseHandler::checkExpiry();
        if ($license_status == TRUE) {
            echo "<h1 align='center' height='500' width='500'>**Your License Has Been Expired.**<hi>";
            die;
        }
        $model = new LoginForm();
        try {
            ...
            if ($model->load(Yii::$app->request->post())) {
                ...
                if ($user) {
                    if ($user->mode_of_authentication === 'database') {
                        if ($model->login()) {
                            if (isset($_REQUEST['cc'])) {
                                Yii::$app->session->set('db_code', $_REQUEST['cc']);
                            }

                            if ($user->hasAccess()) {
                                if (!$user->email_confirmed) {
                                    return $this->redirect('?r=/user-management/auth/change-own-password');
                                } else {
                                    return $this->goHome();
                                }
                            } else {
                                $model->addError('username', UserManagementModule::t('front', 'You are not assigned any Role. Please contact Admin.'));
                            }
                        } else {
                            $model->addError('password', UserManagementModule::t('front', 'Incorrect username or password'));
                        }
                    } else if ($user->mode_of_authentication === 'ad') {
                        $un = Yii::$app->params['ad_domain'] . $model->username;
                        $pw = $model->password;
                        $ldaphost = Yii::$app->params['ad_host'];

                        $username_validate = $model->validateUsername();
                        $password_check = $model->isPasswordBlank();
                        ...
                        if ($username_validate) {
                            if ($password_check) {
                                ...
                                if (ldap_bind($ldapconn, $un, $pw) == TRUE) {
                                    if ($user->hasAccess()) {
                                        if ($model->login()) {
//                                            Yii::$app->user->login($user, 0);
                                            return $this->goHome();
                                        } else {
                                            $model->addError('password', UserManagementModule::t('front', 'User already logged-in from another browser'));
                                        }
                                    } else {
                                        $model->addError('username', UserManagementModule::t('front', 'You are not assigned any Role. Please contact Admin.'));
                                    }
                                ...
                                $model->addError('password', UserManagementModule::t('front', 'Password cannot be blank.'));
                            }
                        } else {
                            $model->addError('username', UserManagementModule::t('front', 'Invalid characters in username'));
                        }
                    } else {
                        $model->addError('username', UserManagementModule::t('front', 'Incorrect mode of authentication'));
                    }
                } else {
                    $model->addError('password', UserManagementModule::t('front', 'Incorrect username or password'));
                }
    ...
    public function actionLogout() {
        if (isset(Yii::$app->session)) {
            $model = new LoginForm();
            $model->setUserFlag(NULL);
            Yii::$app->user->logout();
        }
        return $this->redirect('?r=/user-management/auth/login');
    }
    ...
    public function actionChangeOwnPassword() {
        ...
        if ($user->mode_of_authentication == 'database') {
            ...
            if ($model->load(Yii::$app->request->post()) AND $model->changePassword()) {
                ...
                if ($user->save(TRUE, ['email_confirmed'])) {
                    
                    //if business user, send to profile page, if not submitted
                    if((User::hasRole(['BusinessUser'], false)) && !($user->hasProfileSubmitted())) {
                        Yii::$app->session->setFlash('success', 'Your password is successfully changed. Now complete profile.');
                        
                        return $this->redirect(['/user-management/auth2/profile']);
                    }
                ...
                return $this->redirect(['/user-management/user/view', 'id' => Yii::$app->user->id]);
            }
        } else {
                ...
                if ($this->triggerModuleEvent(UserAuthEvent::AFTER_REGISTRATION, ['model' => $model, 'user' => $user])) {
                    if ($user) {
                        if (Yii::$app->getModule('user-management')->useEmailAsLogin AND Yii::$app->getModule('user-management')->emailConfirmationRequired) {
                            ...
                            $url = 'index.php?r=user-management/user/view&id=' . $user->id;
