...
class AuthHelper {
    ...
    public static function unifyRoute($route) {
        ...
        if ($route) {
            if (strpos($route[0],'?') === FALSE or strpos($route[0],'?') === FALSE) {
            // echo implode('UnifyA- ',$route) .'<br/>';
                $route = Url::to($route);
                $route = trim($route);
              //  echo 'UnifyB - </br>' .$route .'<br/>';
            }
            $route = (array) $route;
            
            if (strpos($route[0],'?') == 0 or strpos($route[0],'?') == 0) {
         //    echo implode('UnifyC- ',$route) .'<br/>';
                $route = Url::toRoute($route);
          //      echo 'UnifyD - </br>' .$route .'<br/>';
               // die();
                $route = trim($route);
            }
 
            if (strpos($route[0], '&') > 0) {
                $route[0] = stristr($route[0], '&', true);
                $route[0] = trim($route[0]);
            }
            
            $route = (array) $route;
          //  echo implode('Unifyfinal- ',$route) .'<br/>';
    ...
    public static function unifyRoute1($route) {
    ...
    private static function getControllerRoutes($module, $namespace, $prefix, &$result) {
        try {
            ...
            $path = Yii::getAlias('@' . $namespace); // Above line was changed by PJ, since it was giving error on Windows machine
